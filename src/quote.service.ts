...
export class QuoteService {
  ...
  private callback: (quote: Quote) => void;
  ...
  generateRandomQuotes(delay: number, callback: (quote: Quote) => void):boolean {
    this.callback = callback;
    this.delay = delay;
    this.isRunning = true;
    callback(this.getRandomQuote());
    this.intervalIndex = setInterval(() => callback(this.getRandomQuote()), delay);
  ...
  continueInterval():boolean {
    this.intervalIndex = setInterval(() => this.callback(this.getRandomQuote()), this.delay);
    return true;
  }
